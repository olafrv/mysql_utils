...
FOUT=/tmp/check_mysql_nagios.output
if [ "$MODE" == "-d" ]
then
	echo "show $ROLE status \G" | $CMD
elif [ "$MODE" == "-n" ]
then

	echo ";" | $CMD 2>&1 >/dev/null
	MYSQL_STATUS=$? 
	if [ $MYSQL_STATUS -eq 0 ]
	then 
		MYSQL_STATUS_MSG="OK ($MYSQL_STATUS)"    
		echo "MYSQL_STATUS: $MYSQL_STATUS_MSG";
	else
	...
	then
		...
		echo "MYSQL_MASTER: $MM_POSITION ($MM_FILE)"
	else
		# MS = MySQL Slave
		...
		echo "MYSQL_SLAVE_IO: $MS_IO_RUN" 
		if [ "$MS_IO_ERRNO" != "0" ]
		then
			echo "MYSQL_SLAVE_IO_ERROR: ${MS_IO_ERRNO}: ${MS_IO_ERR:0:100}"
		fi

		echo "MYSQL_SLAVE_SQL: $MS_SQL_RUN" 
		if [ "$MS_SQL_ERRNO" != "0" ]
		then 
			echo "MYSQL_SLAVE_SQL_ERROR: ${MS_SQL_ERRNO}: ${MS_SQL_ERR:0:100}"
		fi
	...
	echo "Execute: bash check_mysql.sh [-d|-n] [master|slave] IP Port User Password"
fi
